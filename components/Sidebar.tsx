...
export function Sidebar({
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-foreground/10 bg-background/90 px-4 py-3 backdrop-blur md:hidden">
      ...
      {open && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 border-r border-foreground/10 bg-card">
            <button
      ...
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-60 border-r border-foreground/10 bg-card md:block">
        {content}
      </aside>
    </>
  );
